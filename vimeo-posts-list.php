...
function get_vimeo_posts_array() {
    ...
    // Obtenemos el segmento personalizado
    $segment = VIMEO_CUSTOM_SEGMENT;

    // Construimos el patrón REGEXP para MySQL, escapando barras y puntos
    $regexp = "https://vimeo\\.com/([0-9]+|{$segment}/[^[:space:]]+)";

    // Consulta para obtener posts y páginas publicados que contengan al menos una URL de Vimeo
    $query = "
        SELECT ID, post_title, post_content 
        FROM $wpdb->posts 
        WHERE post_status = 'publish'
          AND post_type IN ('post','page')
          AND post_content REGEXP '$regexp'
    ";
    ...
    if ($results) {
        foreach ($results as $post) {
            ...
            // - URLs con el segmento personalizado: https://vimeo.com/<segmento>/...
            $pattern = "/https:\/\/vimeo\.com\/(?:[0-9]+|{$segment}\/[^\s"'<>]+)/";
